...
import {
  ...
  Clock, Check, Copy, Send, TrendingUp, Settings2,
} from 'lucide-react';
...
const apiCall = async (endpoint, options = {}) => {
  const token = authService.getToken?.() || localStorage.getItem('token') || localStorage.getItem('adminToken');
...
export default function SocialMediaPage() {
  ...
  const createReel = async () => {
    ...
    try {
      if (selAccs.length > 1) {
        ...
        showToast(res.message || 'Uploaded!');
      } else {
        await apiCall('/reels', {
          method: 'POST',
          body: JSON.stringify({
        ...
        showToast('Reel created with QUTTR branding!');
      }
      setSelAccs([]);
      setUTitle('');
      setUCaption('');
  ...
  const addAccount = async () => {
    if (!newAcc.username || !newAcc.displayName) return showToast('Fill all fields', 'error');
    try {
  ...
  const simComment = async () => {
    try {
      ...
      if (!rl.reels?.length) return showToast('No reels found. Seed first.', 'error');
  ...
  return (
    <div className="space-y-6">
      {/* Toast */}
      {toast && (
        <div className={`fixed top-20 right-6 z-50 px-4 py-3 rounded-xl text-sm font-medium shadow-lg ${toast.type === 'error' ? 'bg-red-500/90 text-white' : 'bg-emerald-500/90 text-white'}`}>
          {toast.msg}
        </div>
      )}
      ...
      <div className="flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            <Share2 className="w-6 h-6 text-accent-500" />
            Social Media Hub
          </h1>
          <p className="text-sm text-white/40 mt-1">
            10 Accounts · QUTTR Watermark · AI Titles · Auto Replies
          </p>
        </div>
        <button
          onClick={() => { loadAccounts(); loadTab(); }}
          className="flex items-center gap-2 px-3 py-2 rounded-xl bg-white/[0.04] border border-white/[0.06] text-sm text-white/60 hover:text-white transition-colors"
        >
          ...
          return (
            <button
              ...
              className={`flex items-center gap-1.5 px-3.5 py-2 rounded-lg text-xs font-semibold whitespace-nowrap transition-all ${
                tab === t.k
                  ? 'bg-accent-500 text-surface-100 shadow-lg shadow-accent-500/20'
        ...
        <>
            ...
            <div className="space-y-6">
                ...
                {[
                  ...
                  <div key={i} className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-4">
                    <div className="flex items-center justify-between mb-2">
                      <div className={`w-8 h-8 rounded-lg ${s.bg} flex items-center justify-center text-sm`}>●</div>
                      {s.extra && <span className="text-[10px] font-bold text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded-full">{s.extra}</span>}
                    </div>
                  ...
                  <div className="p-4 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
                    {todayStats.accounts?.map((a) => (
                      <div key={a.accountId} className="rounded-xl bg-white/[0.03] border border-white/[0.05] p-3 text-center">
                        ...
                        <p className="text-xl font-bold mt-2" style={{ color: a.color }}>{a.todayUploads}</p>
                        ...
                        <p className="text-[9px] text-white/30 mt-1">{a.todayUploads}/{a.dailyLimit}</p>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] overflow-hidden">
                ...
                <div className="p-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                  ...
                  {reels.length === 0 && (
                    <p className="text-sm text-white/30 col-span-full text-center py-8">No reels yet — go to Setup to seed data</p>
                  )}
                </div>
              </div>
            </div>
          ...
          {tab === 'accounts' && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <p className="text-sm text-white/40">{accounts.length}/10 accounts</p>
                <button
                  onClick={() => setShowAdd(true)}
                  disabled={accounts.length >= 10}
                  className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-accent-500 text-surface-100 text-xs font-bold hover:opacity-90 disabled:opacity-40"
                >
              ...
              {showAdd && (
                <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setShowAdd(false)}>
                  <div className="bg-surface-100 border border-white/[0.08] rounded-2xl w-full max-w-md" onClick={(e) => e.stopPropagation()}>
                    <div className="px-5 py-4 border-b border-white/[0.06] flex items-center justify-between">
                      <h3 className="font-bold">Add Instagram Account</h3>
                    ...
                    <div className="px-5 py-4 border-t border-white/[0.06] flex gap-2 justify-end">
                      <button onClick={() => setShowAdd(false)} className="px-4 py-2 rounded-xl text-sm text-white/50 hover:text-white">Cancel</button>
                      <button onClick={addAccount} className="px-4 py-2 rounded-xl bg-accent-500 text-surface-100 text-sm font-bold">Add Account</button>
                    </div>
                  </div>
                </div>
              )}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] overflow-hidden">
                <div className="px-5 py-4 border-b border-white/[0.06]">
                  <h3 className="text-sm font-bold">🏷️ QUTTR Watermark + AI Title</h3>
                </div>
                <div className="p-5 space-y-4">
                  <p className="text-xs text-white/40">
                    All reels auto-get <span className="text-accent-500 font-bold">QUTTR watermark</span> and <span className="text-emerald-400 font-bold">AI trending titles</span>.
                  </p>
                  <div>
                    <label className="text-xs text-white/40 block mb-1">Title (leave empty for AI)</label>
                    <input
                      className="w-full px-3 py-2.5 rounded-xl bg-white/[0.04] border border-white/[0.08] text-sm focus:outline-none focus:border-accent-500"
                      value={uTitle}
                      onChange={(e) => setUTitle(e.target.value)}
                      placeholder="AI will generate trending title..."
                    />
                  </div>
                  <div>
                    <label className="text-xs text-white/40 block mb-1">Caption (optional)</label>
                    <textarea
                      className="w-full px-3 py-2.5 rounded-xl bg-white/[0.04] border border-white/[0.08] text-sm focus:outline-none focus:border-accent-500 min-h-[80px] resize-y"
                      value={uCaption}
                      onChange={(e) => setUCaption(e.target.value)}
                      placeholder="AI generates caption with QUTTR promo..."
                    />
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] overflow-hidden">
                  <div className="px-5 py-4 border-b border-white/[0.06] flex items-center justify-between">
                    <h3 className="text-sm font-bold">Select Accounts ({selAccs.length})</h3>
                    <button
                      onClick={() =>
                        setSelAccs(
                  ...
                  <div className="p-4 grid grid-cols-2 gap-2">
                    ...
                    {activeAccounts.length === 0 && (
                      <p className="text-xs text-white/30 col-span-2 text-center py-4">No accounts — go to Setup first</p>
                    )}
                  </div>
                </div>
                <button
                  ...
                  className="w-full py-3.5 rounded-xl bg-gradient-to-r from-accent-500 to-accent-700 text-surface-100 font-bold text-sm flex items-center justify-center gap-2 disabled:opacity-40 hover:opacity-90 transition-opacity"
                >
                  <Upload className="w-4 h-4" />
                  {uploading
                    ? 'Creating...'
                    : `Create Reel${selAccs.length > 1 ? 's' : ''} on ${selAccs.length} Account${selAccs.length !== 1 ? 's' : ''}`}
                </button>
              </div>
            </div>
          )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {reels.length === 0 ? (
                <div className="col-span-full text-center py-16 text-white/30">
                  ...
                  <p className="text-xs mt-1">Go to Setup to seed, or Upload to create</p>
                </div>
              ) : (
          ...
          {tab === 'analytics' && (
            <div className="space-y-4">
              ...
              <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] overflow-hidden">
                <div className="px-5 py-4 border-b border-white/[0.06]">
                  <h3 className="text-sm font-bold">Account Performance</h3>
                </div>
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                        ...
                        <tr key={a._id} className="border-b border-white/[0.04] hover:bg-white/[0.02]">
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-2">
                              <div className="w-7 h-7 rounded-lg flex items-center justify-center text-white text-[11px] font-bold" style={{ background: a.color }}>
                                {a.username?.[0]?.toUpperCase()}
                              </div>
                          ...
                          <td className="px-4 py-3 text-xs">{fmt(a.followers)}</td>
                          <td className="px-4 py-3 text-xs font-bold">{a.totalReels}</td>
                          <td className="px-4 py-3"><span className={`text-xs font-bold ${a.todayReels > 0 ? 'text-emerald-400' : 'text-white/30'}`}>{a.todayReels}</span></td>
                          <td className="px-4 py-3 text-xs">{fmt(a.totalViews)}</td>
                          <td className="px-4 py-3 text-xs">{fmt(a.totalLikes)}</td>
                          <td className="px-4 py-3 text-xs" style={{ color: parseFloat(a.avgEngagement) > 5 ? '#34d399' : '#fbbf24' }}>{a.avgEngagement}%</td>
                          <td className="px-4 py-3">
                            <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${a.autoReplyEnabled ? 'bg-emerald-500/15 text-emerald-400' : 'bg-yellow-500/15 text-yellow-400'}`}>
                      ...
                      {accPerf.length === 0 && (
                        <tr><td colSpan={9} className="px-4 py-8 text-center text-white/30 text-sm">No data — seed accounts & reels first</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
          ...
          {tab === 'comments' && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <p className="text-sm text-white/40">{comments.length} comments</p>
                <button onClick={simComment} className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-accent-500 text-surface-100 text-xs font-bold">
              ...
              <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-4">
                {comments.length === 0 ? (
                  <div className="text-center py-12 text-white/30">
                    ...
                    <p>No comments yet</p>
                    <p className="text-xs mt-1">Click Simulate to test auto-replies</p>
                  </div>
                ) : (
                  comments.map((c) => (
                    <div key={c._id} className="flex gap-3 py-3 border-b border-white/[0.04] last:border-0">
                      <div className="w-9 h-9 rounded-full bg-white/[0.06] flex items-center justify-center text-sm flex-shrink-0">👤</div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          <span className="text-xs font-bold">@{c.username}</span>
                          <span className="text-[10px] font-bold px-2 py-0.5 rounded-full ml-auto" style={{ background: `${c.account?.color || '#6C5CE7'}22`, color: c.account?.color || '#6C5CE7' }}>
                            @{c.account?.username}
                          </span>
                        </div>
                        <p className="text-sm text-white/60">{c.text}</p>
                        {c.replied && (
                          <div className="mt-2 pl-3 border-l-2 border-accent-500 bg-accent-500/5 rounded-r-lg py-2 px-3">
                            <p className="text-[10px] font-bold text-accent-500 mb-0.5">🤖 QUTTR AUTO-REPLY</p>
                            <p className="text-xs text-accent-400/80">{c.replyText}</p>
                          </div>
                        )}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] overflow-hidden">
                ...
                <div className="p-5 space-y-3">
                  ...
                  <button
                    onClick={genTitles}
                    disabled={aiLoading}
                    className="w-full py-3 rounded-xl bg-accent-500 text-surface-100 text-sm font-bold disabled:opacity-40"
                  >
                  ...
                  {aiTitles.map((t, i) => (
                    <div
                      key={i}
                      onClick={() => copy(t)}
                      className="flex items-center gap-2 p-3 rounded-xl bg-white/[0.03] border border-white/[0.06] cursor-pointer hover:border-accent-500/40 transition-colors text-xs"
                    >
              ...
              <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] overflow-hidden">
                ...
                <div className="p-5 space-y-3">
                  ...
                  {aiCaption && (
                    <div className="p-4 rounded-xl bg-white/[0.03] border border-white/[0.06] text-xs leading-relaxed whitespace-pre-wrap">
                      {aiCaption}
                      <button onClick={() => copy(aiCaption)} className="mt-3 flex items-center gap-1 text-accent-500 text-xs font-bold">
          ...
          {/* ========== SETUP ========== */}
          {tab === 'setup' && (
            <div className="max-w-lg rounded-2xl border border-white/[0.06] bg-white/[0.02] overflow-hidden">
              <div className="px-5 py-4 border-b border-white/[0.06]">
                <h3 className="text-sm font-bold flex items-center gap-2"><Database className="w-4 h-4 text-accent-500" /> Seed Demo Data</h3>
              </div>
              <div className="p-5 space-y-3">
                <p className="text-xs text-white/40 mb-2">Click each button in order to populate the hub:</p>
                {[
                  { type: 'accounts', label: '1️⃣ Seed 10 Instagram Accounts', desc: 'Creates accounts for all niches' },
                  { type: 'reels', label: '2️⃣ Seed Sample Reels', desc: 'Creates reels with AI titles & metrics' },
                  { type: 'comments', label: '3️⃣ Seed Sample Comments', desc: 'Creates comments with QUTTR auto-replies' },
                ].map((s) => (
                  <div key={s.type} className="flex items-center justify-between p-4 rounded-xl bg-white/[0.03] border border-white/[0.05]">
                    <div>
                ...
                <div className="mt-4 p-4 rounded-xl bg-accent-500/10 border border-accent-500/20">
                  ...
                  <ol className="text-[11px] text-white/50 space-y-1 list-decimal list-inside">
                    <li>Seed accounts → Seed reels → Seed comments</li>
                    <li>Go to Dashboard to see everything</li>
                    <li>Use Upload to create reels on multiple accounts</li>
                  </ol>
                </div>
              </div>
            </div>
